...
const MyPageTest = (data) => {
  ...
  const MypageboxInfo = ({ data, name }) => { // 이름 이메일 주소를 담기위한 컴포넌트
    return (
      <div className="MyPagebox-Info">
        <p style={{ marginLeft: "15px" , color:"blue"}}><b>{name}</b></p>
        {isLogin ? (
          isSocial === "ROLE_SNS" ? (
  ...
  const onPasswordChange = (e) => { // 비밀번호 입력할때 값을 가져오기 위해 만든 onchage 이벤트입니다.
  ...
  return (
    <div className="MyPagebox">
      ...
      {isSocial === "ROLE_SNS" ? 
        ...
        : 
        <Button variant="contained" style={{ marginTop: "10px", fontSize: "20px" }} onClick={() => {window.location.href = "findpassword"}}>
      ...
      !isDelete ? 
       ...
       : 
        <Button variant="contained" style={{ marginTop: "10px", fontSize: "20px" }} onClick={mypagenodelete}>
          취소
        </Button>
      }
      ...
      {yesDelete ? (
        <div className={isDelete ? "MyPagebox-delete" : "MyPagebox-none"}>
          ...
          <Button variant="contained" style={{ fontSize: "20px" }} onClick={() => {setYesDelete(false)}}>
            네, 탈퇴하겠습니다.
          </Button>
        </div>
      ) : (
        <div className="MyPagebox-delete">
          <input placeholder="비밀번호를 입력해주세요" type="password" onChange={onPasswordChange} className="MyPage-Input"></input>
          <Button variant="contained" style={{ marginTop: "10px", fontSize: "20px" }}
            onClick={() => {
              deleteUser(data["data"].userEmail, pw).then((result) => {
                if(result === true){
                  Swal.fire({
                    icon: "success",
                    text: "정상적으로 회원탈퇴되었습니다.",
                  }).then(function(){
