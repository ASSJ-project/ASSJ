...
const MyPageInfo =() => {
    ...
    const[company] = useState(['구글','네이버','배민','요기요','사람인','잡코리아','카리나' , '윈터' , '송하영']);   // 지도클릭가정하기 위해 만들었습니다 추후 지울예정인 코드 조회기록 테스트용코드입니다.
    ...
    useEffect(() => {                                              // 마이페이지(내정보)상태로 들어오면 api를 요청하여 아이디정보를 가져온다
    ...
    const fix1=()=>{                                                  // 메인페이지에서 마커 클릭했을 때 로컬스토리지에 저장한다는 가정 마이페이지 테스트용
      setNumber(number+1)
    ...
    const onChange = (e) => {                               //비밀번호를 입력해주세요 인풋칸에 적으면 useState훅을 통해 적은값을 가져옴
      setPw(e.target.value);
    };

    const checkPw = () =>{                                   // 비밀번호가 동일하면 보이게 안보이게 하려고합니다.
      if(pw==users.phone){
        return(
          setStat(true)
        );
    ...
    return (
        <div className="mypage-myinformations">
            <div className={!stat? 'mypage-myinformation' : "mypage-none"}>현재비밀번호 확인:
            <input placeholder='비밀번호를 입력하세요' type={showPswd ? 'text' : 'password' } onChange={onChange} ></input>
            <div>{showPswd ? <button onClick={()=>setShowPswd(false)}>숨기기</button> : <button onClick={()=>setShowPswd(true)}>보이기</button> }</div>
            <button className='mypage-pwcheck' onClick={checkPw}>확인</button>
            </div>
            <div className={!stat? "mypage-none" : "mypage-myinformation"}>이름 : {users.name}</div>
